...
const PAYMENT_COLORS = {
  airtel: { border: 'border-red-500/40', bg: 'bg-red-900/10', dot: 'bg-red-500', badge: 'Airtel' },
  mtn: { border: 'border-yellow-500/40', bg: 'bg-yellow-900/10', dot: 'bg-yellow-400', badge: 'MTN' },
  zamtel: { border: 'border-green-500/40', bg: 'bg-green-900/10', dot: 'bg-green-500', badge: 'Zamtel' },
  paypal: { border: 'border-blue-500/40', bg: 'bg-blue-900/10', dot: 'bg-blue-400', badge: 'PayPal' },
}
  ...
  return (
    <div className="lux-panel">
      ...
      <div className={`${collapsed ? 'hidden md:block' : 'block'}`}>
        ...
        <div className="divide-y divide-white/5">
          {items.map((item) => (
            <div key={item.key} className="flex gap-4 px-6 py-4">
              <div className="w-14 h-16 lux-panel overflow-hidden flex-shrink-0 relative">
                <img
                  src={item.image}
                  alt={item.name}
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.size && item.size !== 'One Size' && (
                  <p className="text-xs text-gold/50 mt-0.5">{item.size}</p>
                )}
              </div>
        ...
        <div className="px-6 py-4 border-t border-gold/10 space-y-2">
          ...
          <div className="flex justify-between font-medium text-[#f5f0f2] pt-2 border-t border-gold/20">
            <span>Total</span>
...
export default function Checkout() {
  ...
  useEffect(() => {
    if (profile || user) {
      ...
      if (profile?.phone) setMobilePhone(profile.phone)
    }
  }, [profile, user])
  ...
  const saveOrderToSupabase = async (transactionId, method, status = 'Processing') => {
    const orderData = {
    ...
    try {
      const { data, error: dbError } = await supabase
        ...
        .single()
      if (!dbError && data) {
        localStorage.setItem('lafai_last_order', JSON.stringify({ ...data, id: data.id }))
        return data
      }
    } catch {}

    // Fallback to localStorage if Supabase fails (e.g. not configured yet)
    const fallback = {
      id: `ORD-${Date.now()}`,
    ...
    try {
      ...
      localStorage.setItem('lafai_last_order', JSON.stringify(fallback))
    } catch {}
    return fallback
  }
  ...
  const handleMobileMoneyPay = async (method) => {
    ...
    try {
      ...
      if (data.success) {
        await saveOrderToSupabase(data.transactionId || orderId, method, 'Processing')
        clearCart()
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center px-6">
            <p className="font-display text-4xl italic font-light text-gold/40 mb-6">
              Your bag is empty
            </p>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 pt-20">
        <div className="max-w-6xl mx-auto px-6 py-12">
          <h1 className="section-heading gold-sheen mb-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
            ...
            <div className="lg:col-span-3 space-y-8">
              {/* Contact */}
              <div>
                <h2 className="text-xs uppercase tracking-widest text-gold/60 mb-4">
                  Contact Information
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-gold/60 block mb-1">
                      Full Name *
                    </label>
                    <input
                      type="text"
              ...
              <div>
                ...
                {enabledPayments.length === 0 ? (
                  <div className="lux-panel p-6 text-center">
                      ...
                      return (
                        <label
                          ...
                          className={`flex items-center gap-3 p-4 border cursor-pointer transition-all duration-200 ${
                            paymentMethod === method
                              ? `${colors.border || 'border-accent'} ${colors.bg || 'bg-accent/5'}`
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 flex-1">
                            <div className={`w-2 h-2 rounded-full ${colors.dot || 'bg-accent'}`} />
                  ...
                  enabledPayments.includes(paymentMethod) && (
                    <div className="mt-4">
                      ...
                      <input
                        ...
                        placeholder="+260 97..."
                      />
                      <p className="text-[10px] text-[#f5f0f2]/25 mt-1">
                      ...
                      >
                        <PayPalButtons
                          ...
                          onApprove={async (data) => {
                            ...
                            if (result.success) {
                              await saveOrderToSupabase(result.transactionId, 'paypal')
                              clearCart()
                              navigate('/order-success')
                            } else {
